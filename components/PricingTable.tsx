...
export default function PricingTable({
  ...
  subheadline = 'Choose the training path that matches your goals and schedule.',
  tiers = [],
      ...
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {tiers.map(function (tier, i) {
            return (
              <Card key={i} className={'card-hover relative flex flex-col rounded-xl border bg-card ' + (tier.highlighted ? 'border-primary ring-2 ring-primary' : 'border-border')}>
                {tier.highlighted && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-md bg-primary px-3 py-1 text-xs font-semibold uppercase tracking-wide text-primary-foreground">
                    Most Popular
                  </div>
                )}
